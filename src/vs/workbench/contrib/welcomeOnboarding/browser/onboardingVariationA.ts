...
type KeepCodeSignInUiState = 'token-input' | 'validating' | 'success';
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private keepCodeSignInUiState: KeepCodeSignInUiState = 'token-input';
	private keepCodeTokenValue = '';
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "KeepCode AI'a Hoş Geldiniz");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "AI destekli geliştirme için token ile giriş yapın.");
	...
	private _renderDefaultSignInActions(actions: HTMLElement): void {
		// KeepCode AI Token Sign-In
		this._renderKeepCodeTokenSignIn(actions);
	}

	private _renderKeepCodeTokenSignIn(actions: HTMLElement): void {
		switch (this.keepCodeSignInUiState) {
			case 'validating':
				this._renderKeepCodeValidatingState(actions);
				break;
			case 'success':
				// Already handled in parent
				break;
			default:
				this._renderKeepCodeTokenInput(actions);
				break;
		}
	}

	private _renderKeepCodeTokenInput(actions: HTMLElement): void {
		const container = append(actions, $('.onboarding-a-signin-keepcode-token'));

		// 1. Tarayıcıda Giriş Yap Butonu (Büyük ve Şık)
		const browserLoginBtn = append(container, $<HTMLButtonElement>('button.onboarding-a-signin-btn.primary'));
		browserLoginBtn.type = 'button';
		browserLoginBtn.style.width = '100%';
		browserLoginBtn.style.padding = '12px 20px';
		browserLoginBtn.style.marginBottom = '20px';
		browserLoginBtn.style.display = 'flex';
		browserLoginBtn.style.alignItems = 'center';
		browserLoginBtn.style.justifyContent = 'center';
		browserLoginBtn.style.gap = '8px';
		browserLoginBtn.style.borderRadius = '8px';
		browserLoginBtn.style.background = '#4f46e5';
		browserLoginBtn.style.color = '#ffffff';
		browserLoginBtn.style.fontWeight = 'bold';
		browserLoginBtn.style.border = 'none';
		browserLoginBtn.style.cursor = 'pointer';

		const zapIcon = append(browserLoginBtn, $('span'));
		zapIcon.classList.add(...ThemeIcon.asClassNameArray(Codicon.zap));

		const btnText = append(browserLoginBtn, $('span'));
		btnText.textContent = localize('onboarding.signIn.keepcode.browserLogin', "KeepCode AI ile Giriş Yap");

		this.stepDisposables.add(addDisposableListener(browserLoginBtn, EventType.CLICK, () => {
			window.open('https://keepcodeai.com/auth/login', '_blank');
		}));

		// Küçük Ayırıcı Çizgi ve Açıklama
		const divider = append(container, $('.onboarding-a-signin-divider'));
		divider.style.display = 'flex';
		divider.style.alignItems = 'center';
		divider.style.gap = '10px';
		divider.style.margin = '15px 0';

		const divLine1 = append(divider, $('span'));
		divLine1.style.flex = '1';
		divLine1.style.height = '1px';
		divLine1.style.background = 'rgba(255,255,255,0.1)';

		const divText = append(divider, $('span'));
		divText.textContent = localize('onboarding.signIn.keepcode.or', "VEYA");
		divText.style.fontSize = '10px';
		divText.style.color = 'rgba(255,255,255,0.4)';
		divText.style.fontWeight = 'bold';

		const divLine2 = append(divider, $('span'));
		divLine2.style.flex = '1';
		divLine2.style.height = '1px';
		divLine2.style.background = 'rgba(255,255,255,0.1)';

		const description = append(container, $('p.onboarding-a-signin-token-desc'));
		description.textContent = localize('onboarding.signIn.keepcode.desc', "Giriş yaptıktan sonra kopyaladığınız token'ı aşağıya yapıştırın:");
		description.style.fontSize = '12px';
		description.style.color = 'rgba(255,255,255,0.6)';
		description.style.marginBottom = '10px';

		const submitAction = this.stepDisposables.add(new Action(
			'onboarding.signIn.keepcode.submit',
			localize('onboarding.signIn.keepcode.continue', "Doğrula"),
			ThemeIcon.asClassName(Codicon.arrowRight),
			false,
		));

		const inputBox = this.stepDisposables.add(new InputBox(container, undefined, {
			placeholder: localize('onboarding.signIn.keepcode.placeholder', 'Token yapıştırın (kc_live_xxx...)'),
			ariaLabel: localize('onboarding.signIn.keepcode.ariaLabel', "KeepCode AI Token"),
			actions: [submitAction],
			inputBoxStyles: defaultInputBoxStyles,
		}));
		inputBox.value = this.keepCodeTokenValue;
		...
		const message = append(container, $('.onboarding-a-signin-token-message'));

		const validate = (): boolean => {
			this.keepCodeTokenValue = inputBox.value.trim();
			inputBox.element.classList.remove('error');
			message.classList.remove('error', 'info');

			if (!this.keepCodeTokenValue) {
				message.textContent = localize('onboarding.signIn.keepcode.hint', "Token'ınızı dashboard'dan kopyalayabilirsiniz.");
				submitAction.enabled = false;
				return false;
			}

			if (this.keepCodeTokenValue.startsWith('kc_') || this.keepCodeTokenValue.length > 20) {
				message.classList.add('info');
				message.textContent = localize('onboarding.signIn.keepcode.ready', "Token doğrulanmaya hazır.");
			...
			message.textContent = localize('onboarding.signIn.keepcode.invalid', "Geçersiz token formatı.");
			submitAction.enabled = false;
			return false;
		};
		...
		const submit = async () => {
			...
			await this._handleKeepCodeTokenSignIn(this.keepCodeTokenValue);
		};
	...
	private _renderKeepCodeValidatingState(actions: HTMLElement): void {
		const container = append(actions, $('.onboarding-a-signin-keepcode-progress'));
		...
		const message = append(container, $('.onboarding-a-signin-keepcode-progress-message'));
		message.textContent = localize('onboarding.signIn.keepcode.validating', "Token doğrulanıyor...");
	}

	private async _handleKeepCodeTokenSignIn(token: string): Promise<void> {
		this.keepCodeSignInUiState = 'validating';
		this._renderStep();

		const watch = StopWatch.create();

		try {
			// Validate token with KeepCode AI backend
			...
			if (response.ok) {
				...
				await this.configurationService.updateValue('keepcodeai.token', token, ConfigurationTarget.USER);
				await this.configurationService.updateValue('keepcodeai.user', userData, ConfigurationTarget.USER);

				this._userSignedIn = true;
				this.keepCodeSignInUiState = 'success';
				this.telemetryService.publicLog2<InstallChatEvent, InstallChatClassification>('commandCenter.chatInstall', {
					installResult: 'installed',
					installDuration: watch.elapsed(),
					signUpErrorCode: undefined,
					provider: 'keepcode-token'
				});

				this.notificationService.notify({
					severity: Severity.Info,
					message: localize('onboarding.signIn.keepcode.success', "Hoş geldiniz, {0}! Giriş başarılı.", userData.username || userData.email),
				});
			...
			this.keepCodeSignInUiState = 'token-input';
			this._renderStep();
			...
			this.notificationService.notify({
				...
				message: localize('onboarding.signIn.keepcode.error', "Token doğrulanamadı. Lütfen geçerli bir token girin."),
			});
		}
	}
