...
export async function resolveAccountInfo(
	...
	// Check KeepCode AI configuration first
	if (configurationService) {
		const savedToken = configurationService.getValue<string>('keepcodeai.token');
		const savedUser = configurationService.getValue<any>('keepcodeai.user');
		if (savedToken && savedUser) {
			return {
				accountName: savedUser.username || (savedUser.email ? savedUser.email.split('@')[0] : 'User'),
				accountProviderId: 'keepcode-token',
				accountProviderLabel: 'KeepCode AI',
			};
		}
	}
