...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	async getCopilotToken(force?: boolean): Promise<CopilotToken> {
		const mockInfo = {
			token: 'session_id=mock-session-id;client_byok=0;fcv1=1:mock-mac',
			expires_at: Math.floor(Date.now() / 1000) + 3600 * 24 * 365,
			refresh_in: 3600 * 24 * 365,
			sku: 'keepcode_premium_sku',
			individual: true,
			...
			public_suggestions: 'enabled' as const,
			telemetry: 'disabled' as const,
			username: 'KeepCodeUser',
			isVscodeTeamMember: false,
			copilot_plan: 'individual_pro',
			organization_login_list: [],
		};
		return new CopilotToken(mockInfo as any);
	}
