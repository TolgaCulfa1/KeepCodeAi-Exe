...
async function networkRequest(
	...
	const isKeepCodeChat = body && (body.messages || body.input);
	if (isKeepCodeChat) {
		const deepseekModel = endpoint.model === 'keepcode-reasoner' ? 'deepseek-reasoner' : 'deepseek-chat';
		
		let messages = body.messages;
		if (!messages && body.input) {
			messages = body.input.map((item: any) => {
				if (item.role) return item;
				return { role: item.author || 'user', content: item.text || '' };
			});
		}
		...
		let mappedMessages = messages.map((msg: any) => {
			...
			let role = msg.role;
			if (deepseekModel === 'deepseek-reasoner' && role === 'system') {
				role = 'user';
			}
			return { role, content };
		});
		...
		if (deepseekModel !== 'deepseek-reasoner') {
			...
			if (body.top_p !== undefined) dsBody.top_p = body.top_p;
			if (body.tools) {
				dsBody.tools = body.tools;
			}
		}

		// Dynamically get user token from vscode.authentication
		const vscode = require('vscode');
		const authSession = await vscode.authentication.getSession('github', [], { silent: true });
		if (!authSession) {
			vscode.window.showErrorMessage(vscode.l10n.t('Lütfen önce KeepCode AI hesabınızla giriş yapın.'));
			throw new Error('KeepCode AI: Not authenticated');
		}

		const dsHeaders: ReqHeaders = {
			'Content-Type': 'application/json',
			'Authorization': `Bearer ${authSession.accessToken}`
		};

		const dsRequest: FetchOptions = {
			callSite: `network-request-deepseek`,
			method: 'POST',
			headers: dsHeaders,
			json: dsBody,
		...
		if (cancelToken) {
			...
			dsRequest.signal = abort.signal;
		}

		return fetcher.fetch('https://code.keepcodeai.com/api/chat/completions', dsRequest).then(response => {
			const originalText = response.text;
			response.text = async function() {
				let text = await originalText.call(response);
				if (text.includes('"reasoning_content"')) {
					text = text.replace(/"reasoning_content"/g, '"reasoning_text"');
				}
				return text;
			};
