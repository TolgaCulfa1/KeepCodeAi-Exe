...
export class ModelMetadataFetcher extends Disposable implements IModelMetadataFetcher {
	...
	private async _fetchModels(force?: boolean): Promise<void> {
		...
		const mockChatModel: IModelAPIResponse = {
			id: 'keepcode-chat',
			vendor: 'KeepTG',
			name: 'KeepCode 2.1',
			...
			supported_endpoints: ['/chat/completions' as any]
		};

		const mockReasonerModel: IModelAPIResponse = {
			id: 'keepcode-reasoner',
			vendor: 'KeepTG',
			name: 'KeepCode 2.1 (Reasoner)',
			model_picker_enabled: true,
		...
		const hydratedChatModel = await this._hydrateResolvedModel(mockChatModel);
		const hydratedReasonerModel = await this._hydrateResolvedModel(mockReasonerModel);

		this._copilotUtilityModel = hydratedChatModel;
		
		this._familyMap.set('keepcode-chat', [hydratedChatModel]);
		this._familyMap.set('keepcode-reasoner', [hydratedReasonerModel]);
