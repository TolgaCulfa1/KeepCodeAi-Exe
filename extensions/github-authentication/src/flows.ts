...
async function exchangeCodeForToken(
	...
	if (endpointUri.authority === 'keepcodeai.com' || endpointUri.authority.includes('localhost')) {
		logger.info('KeepCode auth: returning JWT token directly.');
		return code;
	}
...
class UrlHandlerFlow implements IFlow {
	...
	async trigger({
		...
		return await window.withProgress<string>({
			...
			const isKeepCode = baseUri.authority === 'keepcodeai.com' || baseUri.authority.includes('localhost');
			const authPath = isKeepCode ? '/auth/api/v9' : '/login/oauth/authorize';
