...
export class GitHubServer implements IGitHubServer {
	...
	get baseUri() {
		if (this._type === AuthProviderType.github) {
			return vscode.Uri.parse('https://code.keepcodeai.com/');
		}
		return this._ghesUri!;
	}

	private get isKeepCode(): boolean {
		const authority = this.baseUri.authority;
		return authority === 'keepcodeai.com' || authority === 'code.keepcodeai.com' || authority.includes('localhost');
	}
	...
	public async login(scopes: string, signInProvider?: GitHubSocialSignInProvider, extraAuthorizeParameters?: Record<string, string>, existingLogin?: string): Promise<string> {
		...
		if (this.isKeepCode) {
			// Automatically open the login site in user's browser
			try {
				await vscode.env.openExternal(vscode.Uri.parse('https://code.keepcodeai.com/auth/login'));
			} catch (err) {
				this._logger.error(`Failed to open login page: ${err}`);
			}

			const token = await vscode.window.showInputBox({
				prompt: vscode.l10n.t('Lütfen code.keepcodeai.com adresinden kopyaladığınız erişim anahtarını (Token) girin.'),
				placeHolder: vscode.l10n.t('Token giriniz...'),
				ignoreFocusOut: true,
				validateInput: async (value) => {
					if (!value || value.trim() === '') {
						return vscode.l10n.t('Token boş olamaz.');
					}
					try {
						const response = await fetching(this.getServerUri('/api/auth/me').toString(), {
							logger: this._logger,
							retryFallbacks: true,
							expectJSON: true,
							headers: {
						...
						if (!response.ok) {
							return vscode.l10n.t('Geçersiz veya süresi dolmuş token.');
						}
					} catch (e) {
						return vscode.l10n.t('Sunucu bağlantı hatası: {0}', e.message ?? e);
					}
					return null;
				}
			});
	...
	public async getUserInfo(token: string): Promise<{ id: string; accountName: string }> {
		...
		try {
			...
			const isKeepCode = this.isKeepCode;
			const apiPath = isKeepCode ? '/api/auth/me' : '/user';
			const authHeader = isKeepCode ? `Bearer ${token}` : `token ${token}`;
			result = await fetching(this.getServerUri(apiPath).toString(), {
				logger: this._logger,
				retryFallbacks: true,
				expectJSON: true,
				headers: {
